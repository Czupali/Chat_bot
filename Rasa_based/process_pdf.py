...
def extract_pdf_text(pdf_file, output_dir="processed_txt", save_to_file=False):
        ...
        try:
            with open(pdf_path, "rb") as f:
                ...
                if not header.startswith(b"%PDF-"):
                    ...
                    return f"Hiba: A fájl nem érvényes PDF: {pdf_path}", None
        except Exception as e:
            logger.error(f"Nem sikerült megnyitni a fájlt: {e}")
            return f"Hiba: Nem sikerült megnyitni a fájlt: {e}", None
    else:
        pdf_path = pdf_file
    ...
    if not os.path.exists(pdf_path):
        ...
        return f"Hiba: A fájl nem található: {pdf_path}", None

    file_size = os.path.getsize(pdf_path)
    logger.info(f"File size: {file_size} bytes")
    if file_size == 0:
        logger.error(f"File is empty: {pdf_path}")
        return f"Hiba: A fájl üres: {pdf_path}", None
    ...
    try:
        ...
        if save_to_file:
            ...
            logger.info(f"PDF text saved to: {output_file}")
